...
public class Main {
            ...
            for (String species : speciesList) {
                ...
                for (Animal a : animals) {
                    ...
                    if (a.getSpecies().equalsIgnoreCase(species)) {
                        pw.println("Name: " + a.getName() + ", Age: " + a.getAge() + detail);
                        count++;
                    }
                }
    ...
    public static void main(String[] args) {
        ...
        try (Scanner sc = new Scanner(file1)) {
            while (sc.hasNextLine()) {
                ...
                if (parts.length >= 1) {
                    ...
                    switch (speciesKey) {
                        case "Hyena":
                            myAnimal = new Hyena(animalName, anAge);
                            break;
                        case "Lion":
                            myAnimal = new Lion(animalName, anAge);
                            break;
                        case "Tiger":
                            myAnimal = new Tiger(animalName, anAge);
                            break;
                        case "Bear":
                            myAnimal = new Bear(animalName, anAge);
                            break;
                        default:
                            myAnimal = new Animal(animalName, anAge, speciesKey);
                    }
        ...
        // 2. Inside your animal processing loop (where you have aSpecies):
        String aSpecies = "Lion"; // Example species parsed from file [cite: 2]
        String animalName = "Unknown";

        // Check if we have names for this species
        if (nameBank.containsKey(aSpecies) && !nameBank.get(aSpecies).isEmpty()) {
            // Remove the first name so it isn't reused
            animalName = nameBank.get(aSpecies).remove(0);
        }
